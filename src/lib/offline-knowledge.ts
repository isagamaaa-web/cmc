...
export function offlineAnswer(question: string): string | null {
  ...
  if (matched) {
    return `${matched.title} — ${matched.description} Open 24/7; call ${CLINIC.phones[0]} to arrange it.`;
  }
  ...
  if (has("doctor", "gebeyehu", "specialist", "who is")) {
    return `${CLINIC.doctor} is our ${CLINIC.role}, leading care at ${CLINIC.name}. Consultations are available 24/7.`;
  }

  if (has("lab", "test", "tests", "panel", "panels")) {
    return [
      "Our laboratory panels:",
      ...LAB_PANEL_ITEMS.map((s) => `• ${s.title} — ${s.description}`),
    ].join("\n");
  }
  ...
  if (has("service", "services", "offer", "provide", "do you have")) {
    return [
      ...
      ...CLINICAL_SERVICE_ITEMS.map((s) => `• ${s.title} — ${s.description}`),
      "",
      "Laboratory panels:",
      ...LAB_PANEL_ITEMS.map((s) => `• ${s.title} — ${s.description}`),
    ].join("\n");
